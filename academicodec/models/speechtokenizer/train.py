...
from academicodec.models.speechtokenizer.dataset import (
    SpeechTokenizerDataset,
    collate_fn,
)
...
def get_args():
    ...
    parser.add_argument(
        "--seed", type=int, default=6666, help="seed for initializing trainig. "
    )
    parser.add_argument(
        "--cudnn_deterministic",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=160000,
        help="audio_duraion_in_sec for training",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="speech tokenizer config path",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1,
        help="hyper-parameter for distillation loss",
    )
...
def main_worker(local_rank, args):
    ...
    optimizer_d = torch.optim.AdamW(
        itertools.chain(stft_disc.parameters(), msd.parameters(), mpd.parameters()),
        lr=3e-4,
        betas=(0.5, 0.9),
    )
    ...
    for epoch in range(args.st_epoch, args.N_EPOCHS + 1):
        ...
        for x in tqdm(train_loader):
            ...
            for optimizer_idx in [0, 1]:
                x_wav = get_input(x)
                G_x, commit_loss, feature = speechtokenizer(
                    x_wav
                )  # feature(bs, timesteps, 768)
                if args.do_distillation:
                    feature, teacher_cont, teacher_unit = (
                        feature.to(args.device),
                        teacher_cont.to(args.device),
                        teacher_unit.to(args.device),
                    )
                    cont_loss, pseudo_loss = loss_distillation(
                        feature, teacher_cont, teacher_unit, args.device
                    )
                ...
                if optimizer_idx == 0:
                    ...
                    total_loss_g, rec_loss, adv_g_loss, feat_loss, d_weight = loss_g(
                        ...
                        distillation_pseudo_loss=pseudo_loss,
                        args=args,
                    )
